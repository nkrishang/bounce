...
type Step = 'idle' | 'selling' | 'success';
...
export function useSellPosition() {
  ...
  const sellPosition = useCallback(
    async (betId: number, minUsdcOut: bigint, sellData: `0x${string}`) => {
      ...
      try {
        ...
        const { hash } = await sendAndConfirm(publicClient, () =>
          walletClient.writeContract({
            ...
            args: [BigInt(betId), minUsdcOut, sellData],
            account: address,
          }),
        );
  ...
  return { sellPosition, reset, isLoading, step, error };
}
