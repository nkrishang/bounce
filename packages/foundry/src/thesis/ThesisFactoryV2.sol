...
contract ThesisFactoryV2 {
    ...
    event ThesisCreated(
        address indexed safe, address indexed settlement, address funder, address proposer, uint256 totalCapital
    );
    ...
    function createThesis(address safe, address funder, address proposer, uint256 totalCapital)
        external
        returns (ThesisSettlementV2 settlement)
    {
        return createThesisWithParams(
            safe, funder, proposer, totalCapital, DEFAULT_PROPOSER_CAPITAL_BPS, DEFAULT_PROPOSER_PROFIT_SHARE_BPS
        );
    }
    ...
    function createThesisWithParams(
        ...
        settlement = new ThesisSettlementV2(
            safe, address(manager), funder, proposer, USDC, totalCapital, proposerCapitalBps, proposerProfitShareBps
        );
